...
{   
    public partial class Billing : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //coffee pries
            lat = countAllRows("select price from table1 where id = 1");
            ...
            cappu = 170;
            afri = 150;
            mTea = countAllRows("select price from table1 where id = 5");
            cTea = countAllRows("select price from table1 where id = 6");
            //coffee pries

            //cake prices
            cCake = countAllRows("select price from table1 where id = 2");
            rValvet = 120;
            bFor = 130;
            bCream = 190;
            lChoco = 150;
            kChoco = 140;
            cheese = countAllRows("select price from table1 where id = 7");
            rain = countAllRows("select price from table1 where id = 8");
            //cake prices
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //countAllRows("select count(@name) from table1", countTableRows);

            //assignPriceForDrinks();
           // assignPriceForCake();
        }

        public void assignPriceForDrinks()
        {
          
        }

        public void assignPriceForCake()
        {
            
        }

        public double countAllRows(string query)
        {
            conn.Open();
            SqlCommand command = new SqlCommand(query, conn);
            double variable = Convert.ToDouble(command.ExecuteScalar().ToString());
            conn.Close();
            return variable;
